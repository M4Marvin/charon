...
export function ConnectionSection({ isStreaming }: SectionProps) {
  ...
  const { data: models, isLoading: modelsLoading, isError: modelsError, refetch: refetchModels } = useProviderModels(settings?.defaultProviderId ?? "");
  const testConnection = useTestProviderConnection();

  const [testResult, setTestResult] = useState<{ ok: boolean; latencyMs: number; modelCount: number; error?: string } | null>(null);
  ...
  const handleProviderChange = useCallback(
    (providerId: string) => {
      updateUserSettings.mutate({ defaultProviderId: providerId, defaultSelectedModel: null, defaultPresetId: null });
      setTestResult(null);
      setModelInput("");
    },
    [updateUserSettings],
  ...
  const handleTest = useCallback(() => {
    ...
    testConnection.mutate(selectedProviderId, {
      ...
      onError: () => setTestResult({ ok: false, latencyMs: 0, modelCount: 0, error: "Connection failed" }),
    });
    ...
    <div className="flex flex-col gap-5">
      ...
      <Field className="space-y-1.5">
        ...
        <div className="flex gap-2">
          <Select value={selectedProviderId} onValueChange={handleProviderChange} disabled={isStreaming}>
      ...
      {testResult && (
        <div
          role="status"
          className="rounded-lg border border-white/5 px-3 py-2 text-xs"
        >
          {testResult.ok ? (
      ...
      <Field className="space-y-1.5">
        ...
        <div className="flex gap-2">
          <Select value={selectedModel} onValueChange={handleModelChange} disabled={isStreaming || !selectedProviderId}>
            <SelectTrigger id="cs-model" className="flex-1">
              <SelectValue placeholder="Select model" />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
                {modelsLoading && <div className="px-2 py-1.5 text-xs text-muted-foreground">Loading models…</div>}
                {modelsError && <div className="px-2 py-1.5 text-xs text-red-400">Failed to load models</div>}
                {models?.map((m) => (
      ...
      <Field className="space-y-1.5">
        ...
        <Select value={selectedPresetId || "_none"} onValueChange={handlePresetChange} disabled={isStreaming}>
