...
describe("getRootId", () => {
  ...
  it("returns the id of a node with parentLocalId === null", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { parentLocalId: null }));
    expect(getRootId(tree)).toBe(0);
  });

  it("treats parentLocalId === undefined as a root", () => {
    const tree = createTree();
    tree.set(7, { localId: 7, children: [], role: "user", content: "x" } as unknown as ChatMessage);
...
describe("getActiveLeafId", () => {
  ...
  it("returns the root when root has no selectedChildLocalId", () => {
    const tree = createTree();
    tree.set(0, makeNode(0));
    expect(getActiveLeafId(tree)).toBe(0);
  });

  it("follows the selectedChildLocalId chain to the deepest leaf", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { selectedChildLocalId: 1 }));
    tree.set(1, makeNode(1, { parentLocalId: 0, selectedChildLocalId: 2 }));
    tree.set(2, makeNode(2, { parentLocalId: 1, selectedChildLocalId: 3 }));
    tree.set(3, makeNode(3, { parentLocalId: 2 }));
    expect(getActiveLeafId(tree)).toBe(3);
  });

  it("throws on a selectedChildLocalId cycle", () => {
    const tree = createTree();
    const a = makeNode(0, { selectedChildLocalId: 1 });
    const b = makeNode(1, { parentLocalId: 0, selectedChildLocalId: 0 });
    tree.set(0, a);
    tree.set(1, b);
  ...
  it("accepts an explicit rootId override", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1], selectedChildLocalId: 1 }));
    tree.set(1, makeNode(1, { parentLocalId: 0, selectedChildLocalId: 2 }));
    tree.set(2, makeNode(2, { parentLocalId: 1 }));
...
describe("getActivePath", () => {
  ...
  it("returns [root] when root has no selectedChildLocalId and no leafId is given", () => {
    const tree = createTree();
    tree.set(0, makeNode(0));
    expect(getActivePath(tree)).toHaveLength(1);
    expect(getActivePath(tree)[0].localId).toBe(0);
  });
  ...
  it("walks leaf→root via parentLocalId and returns root→leaf order", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { selectedChildLocalId: 1 }));
    tree.set(1, makeNode(1, { parentLocalId: 0, selectedChildLocalId: 2 }));
    tree.set(2, makeNode(2, { parentLocalId: 1, selectedChildLocalId: 3 }));
    ...
    expect(path.map((n) => n.localId)).toEqual([0, 1, 2, 3]);
  });
  ...
  it("honors an explicit activeLeafId (can view non-selected branch)", () => {
    ...
    tree.set(2, makeNode(2, { parentLocalId: 0 }));
    tree.set(3, makeNode(3, { parentLocalId: 1 }));
    const path = getActivePath(tree, 3);
    expect(path.map((n) => n.localId)).toEqual([0, 1, 3]);
  });

  it("throws on a parentLocalId cycle", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1, 2] }));
    const a = makeNode(1, { parentLocalId: 2 });
    const b = makeNode(2, { parentLocalId: 1 });
    tree.set(1, a);
    tree.set(2, b);
...
describe("getNextSiblingId / getPrevSiblingId", () => {
  function setupFamily(): ChatTree {
    ...
    tree.set(1, makeNode(1, { parentLocalId: 0 }));
    tree.set(2, makeNode(2, { parentLocalId: 0 }));
    tree.set(3, makeNode(3, { parentLocalId: 0 }));
    return tree;
  }
  ...
  it("returns null for a node whose parent is missing", () => {
    const tree = createTree();
    tree.set(1, makeNode(1, { parentLocalId: 999 }));
  ...
  it("returns null when the node is missing from parent.children (corrupt)", () => {
    ...
    tree.set(5, makeNode(5, { parentLocalId: 0 }));
...
describe("getSiblings", () => {
  ...
  it("returns just the node when the parent is missing", () => {
    const tree = createTree();
    const n = makeNode(1, { parentLocalId: 999 });
    tree.set(1, n);
  ...
  it("returns all children of the parent in order, including the node itself", () => {
    ...
    const b = makeNode(1, { parentLocalId: 0 });
    const c = makeNode(2, { parentLocalId: 0 });
    const d = makeNode(3, { parentLocalId: 0 });
    tree.set(0, a);
    tree.set(1, b);
    tree.set(2, c);
    tree.set(3, d);
...
describe("addChild", () => {
  it("appends the node under the parent, sets parentLocalId, and auto-selects it", () => {
    const tree = createTree();
    ...
    expect(child.parentLocalId).toBe(0);
    expect(child.children).toEqual([]);
    expect(child.selectedChildLocalId).toBeNull();
  ...
  it("resets a pre-existing selectedChildLocalId on the new node", () => {
    const tree = createTree();
    tree.set(0, makeNode(0));
    const node = addChild(tree, 0, makeNode(1, { selectedChildLocalId: 999 }));
    expect(node.selectedChildLocalId).toBeNull();
  });
...
describe("addSibling", () => {
  it("inserts the node after nodeId in the parent children list", () => {
    ...
    expect(sib.parentLocalId).toBe(0);
    expect(sib.children).toEqual([]);
    expect(sib.selectedChildLocalId).toBeNull();
    expect(tree.get(0)!.children).toEqual([1, 3, 2]);
  });

  it("does NOT change parent.selectedChildLocalId (no auto-select)", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1], selectedChildLocalId: 1 }));
    tree.set(1, makeNode(1, { parentLocalId: 0 }));
  ...
  it("appends to the end when nodeId exists in the tree but is missing from parent.children", () => {
    ...
    tree.set(1, makeNode(1, { parentLocalId: 0 }));
    tree.set(99, makeNode(99, { parentLocalId: 0 }));
    addSibling(tree, 99, makeNode(2, { parentLocalId: 0 }));
...
describe("selectChild", () => {
  it("updates parent.selectedChildLocalId", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1, 2] }));
    tree.set(1, makeNode(1, { parentLocalId: 0 }));
    tree.set(2, makeNode(2, { parentLocalId: 0 }));
...
describe("deleteSubtree", () => {
  ...
  it("re-points selectedChildLocalId to the next sibling when deleting the selected leaf", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1, 2, 3], selectedChildLocalId: 1 }));
    tree.set(1, makeNode(1, { parentLocalId: 0 }));
    tree.set(2, makeNode(2, { parentLocalId: 0 }));
    tree.set(3, makeNode(3, { parentLocalId: 0 }));
    deleteSubtree(tree, 1);
  ...
  it("falls back to the previous sibling when there is no next sibling", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1, 2], selectedChildLocalId: 2 }));
    tree.set(1, makeNode(1, { parentLocalId: 0 }));
    tree.set(2, makeNode(2, { parentLocalId: 0 }));
  ...
  it("sets selectedChildLocalId to null when deleting the only child", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1], selectedChildLocalId: 1 }));
    tree.set(1, makeNode(1, { parentLocalId: 0 }));
    deleteSubtree(tree, 1);
    expect(tree.get(0)!.selectedChildLocalId).toBeNull();
    expect(tree.get(0)!.children).toEqual([]);
  });

  it("leaves selectedChildLocalId unchanged when the deleted node was not selected", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1, 2], selectedChildLocalId: 2 }));
    tree.set(1, makeNode(1, { parentLocalId: 0 }));
    tree.set(2, makeNode(2, { parentLocalId: 0 }));
  ...
  it("recursively deletes all descendants of an internal node", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1, 2], selectedChildLocalId: 1 }));
    tree.set(1, makeNode(1, { parentLocalId: 0, children: [3, 4] }));
    tree.set(2, makeNode(2, { parentLocalId: 0 }));
    tree.set(3, makeNode(3, { parentLocalId: 1, children: [5] }));
    tree.set(4, makeNode(4, { parentLocalId: 1 }));
    tree.set(5, makeNode(5, { parentLocalId: 3 }));
  ...
  it("removes a root with no parentLocalId update", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1] }));
    tree.set(1, makeNode(1, { parentLocalId: 0 }));
...
describe("treeFromNodes", () => {
  it("builds a Map from a flat array", () => {
    const nodes = [makeNode(0), makeNode(1, { parentLocalId: 0 })];
  ...
  it("clones the children array (mutating the source does not affect the tree)", () => {
    ...
    const c1 = makeNode(1, { parentLocalId: 0 });
    const c2 = makeNode(2, { parentLocalId: 0 });
...
describe("treeToNodes", () => {
  it("returns all values in insertion order", () => {
    ...
    tree.set(1, makeNode(1, { parentLocalId: 0 }));
    tree.set(2, makeNode(2, { parentLocalId: 1 }));
    expect(treeToNodes(tree).map((n) => n.localId)).toEqual([0, 1, 2]);
  });
...
describe("treeToActivePath", () => {
  ...
  it("returns only the selected branch root→leaf, omitting siblings", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { selectedChildLocalId: 1 }));
    tree.set(1, makeNode(1, { parentLocalId: 0, selectedChildLocalId: 3 }));
    tree.set(2, makeNode(2, { parentLocalId: 0 }));
    tree.set(3, makeNode(3, { parentLocalId: 1, selectedChildLocalId: 4 }));
    tree.set(4, makeNode(4, { parentLocalId: 3 }));
    expect(treeToActivePath(tree).map((n) => n.localId)).toEqual([0, 1, 3, 4]);
  });
});
...
describe("validateTree", () => {
  it("passes for a simple valid tree", () => {
    const tree = treeFromNodes([
      ...
      makeNode(1, { parentLocalId: 0, children: [2] }),
      makeNode(2, { parentLocalId: 1 }),
    ]);
  ...
  it("errors when a child parentLocalId does not match its parent", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1] }));
    tree.set(1, makeNode(1, { parentLocalId: 999 }));
    const result = validateTree(tree);
    expect(result.valid).toBe(false);
    expect(result.errors.some((e) => /parentLocalId 999, expected 0/.test(e))).toBe(true);
  });

  it("errors when selectedChildLocalId is not in the children list", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1], selectedChildLocalId: 999 }));
    tree.set(1, makeNode(1, { parentLocalId: 0 }));
    const result = validateTree(tree);
    expect(result.valid).toBe(false);
  ...
  it("errors on a cycle in the children graph", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1], selectedChildLocalId: 1 }));
    tree.set(1, makeNode(1, { parentLocalId: 0, children: [0] }));
    const result = validateTree(tree);
    expect(result.valid).toBe(false);
  ...
  it("passes for a valid branching tree", () => {
    const tree = createTree();
    tree.set(0, makeNode(0, { children: [1, 2], selectedChildLocalId: 2 }));
    tree.set(1, makeNode(1, { parentLocalId: 0, children: [3] }));
    tree.set(2, makeNode(2, { parentLocalId: 0, children: [4], selectedChildLocalId: 4 }));
    tree.set(3, makeNode(3, { parentLocalId: 1 }));
    tree.set(4, makeNode(4, { parentLocalId: 2 }));
    const result = validateTree(tree);
    expect(result.valid).toBe(true);
    expect(result.errors).toEqual([]);
  });
...
describe("integration: swipe / regenerate workflow", () => {
  ...
  it("supports the full SillyTavern swipe flow and stays valid at every step", () => {
    ...
    expect(validateTree(tree).valid).toBe(true);
    expect(getActivePath(tree).map((n) => n.localId)).toEqual([0, 1, 2]);

    const swipeB = addSibling(tree, 2, makeRoleNode(3, "assistant", "reply B (swipe)"));
    expect(validateTree(tree).valid).toBe(true);
    expect(tree.get(1)!.children).toEqual([2, 3]);
    expect(tree.get(1)!.selectedChildLocalId).toBe(2);
    expect(getActivePath(tree).map((n) => n.localId)).toEqual([0, 1, 2]);

    selectChild(tree, 1, 3);
    expect(getActivePath(tree).map((n) => n.localId)).toEqual([0, 1, 3]);
    expect(validateTree(tree).valid).toBe(true);

    addChild(tree, swipeB.localId, makeRoleNode(4, "user", "follow-up"));
    expect(getActivePath(tree).map((n) => n.localId)).toEqual([0, 1, 3, 4]);
    expect(validateTree(tree).valid).toBe(true);
    ...
    const activeBefore = getActivePath(tree).map((n) => n.localId);
    const activeAfter = getActivePath(roundTrip).map((n) => n.localId);
