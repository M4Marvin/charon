...
export function EntryEditorSheet(props: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (keys.length === 0 && !constant) { setError("At least one keyword is required (or set Constant)."); return; }
    const orderNum = Number.parseInt(order, 10);
    if (Number.isNaN(orderNum)) { setError("Order must be a number."); return; }
    try {
      if (mode === "create") {
        await createMutation.mutateAsync({ comment: comment.trim(), content: content.trim(), key: keys, keysecondary: secondary });
      } else {
        await updateMutation.mutateAsync({ entryId: initial!.id, uid: initial!.uid,
          data: { ...initial!.data, comment: comment.trim(), content: content.trim(), key: keys, keysecondary: secondary, order: orderNum, disable, constant } });
      }
      onClose();
    } catch (err) {
    ...
    <Sheet open onOpenChange={(o) => !o && onClose()}>
      <SheetContent side="right" className="sm:max-w-lg w-full overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>Keywords activate the entry. Content is injected into the prompt when matched.</SheetDescription>
        </SheetHeader>
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          <div className="space-y-1.5">
            <Label htmlFor="comment">Comment</Label>
            <Input id="comment" value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Short description for the author" disabled={isPending} />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <Textarea id="content" value={content} onChange={(e) => setContent(e.target.value)} placeholder="The lore text injected into the prompt..." rows={6} required disabled={isPending} />
            <p className="text-3 text-xs text-right">~{tokenCount} tokens</p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label htmlFor="order">Order</Label>
              <Input id="order" value={order} onChange={(e) => setOrder(e.target.value)} disabled={isPending} />
            </div>
