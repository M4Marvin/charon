...
function FieldRow({ label, value, nil }: { label: string; value: React.ReactNode; nil?: boolean }) {
  return (
    <div className="flex items-start py-1 text-xs">
      ...
      <span
        className={nil ? "text-muted-foreground/50 italic" : "text-foreground font-mono break-all"}
      >
        {value}
      </span>
    </div>
          ...
          <div className="flex-[1_1_400px] min-w-[300px]">
            ...
            {hasError && (
              <Section title="Error">
                <div className="text-destructive font-mono text-sm">
                  {String((demo as unknown as Record<string, unknown>).error)}
                </div>
              </Section>
            )}
            ...
            {hasData && (
              <>
                {(() => {
                  const d = demo!;
                  const { settings, persona, loreEntries, provider } = d;
                  return (
                    <>
                      {/* ── Chat Info ── */}
                        ...
                        <div className="space-y-1">
                          <FieldRow
                            ...
                            value={
                              <span>
                                {persona.name}
                                {persona.isFallback && <Badge variant="info">account name</Badge>}
                                {!persona.isFallback && <Badge variant="set">persona</Badge>}
                              </span>
                            }
                          />
                          <FieldRow
                        ...
                        {provider ? (
                          <>
                            ...
                            {Object.keys(provider.preset).length > 0 && (
                              <div className="mt-3">
                                <div className="font-semibold mb-1 text-sm text-foreground">
                                  Preset Overrides
                                </div>
                            ...
                            {loreEntries.map((entry, i) => (
                              ...
                              >
                                <div className="flex flex-wrap gap-2 items-center mb-1">
                                  <strong className="text-foreground">{entry.displayName}</strong>
                      ...
                      <Section title="What This Shows">
                        <ol className="text-sm m-0 pl-5 leading-relaxed text-foreground">
                          ...
                          <li>
                            ...
                            Falls back to account name if no persona is configured or the persona
                            was deleted.
                          </li>
                          <li>
                            <strong>Provider</strong> — resolved from <code>defaultProviderId</code>
                            . API key is never sent to the client (only <code>hasApiKey</code>{" "}
                            boolean).
                          </li>
                          <li>
                          ...
                          <li>
                            <strong>Character overrides</strong> — per-chat copies of character
                            fields, stored on the <code>chats</code> row. These overlay the
