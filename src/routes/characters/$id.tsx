    ...
    <main className="mx-auto max-w-[1200px] px-4 py-8">
      ...
      <div className="mb-8 space-y-6">
        <div className="flex flex-col sm:flex-row gap-6">
          ...
          <div className="flex flex-col min-w-0 justify-center gap-2">
            ...
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-2">
              {data.creator ? <span className="flex items-center gap-1"><User className="size-3.5" />{data.creator}</span> : null}
              <Badge variant="secondary" className="text-xs font-normal">{character.spec}</Badge>
              <Badge variant="outline" className="text-xs font-normal">v{character.specVersion}</Badge>
              {data.character_version ? <Badge variant="outline" className="text-xs font-normal">card v{data.character_version}</Badge> : null}
            </div>
            {data.tags.length > 0 ? (
              <div className="flex flex-wrap gap-1">
                {data.tags.map((tag) => <Badge key={tag} variant="secondary" className="text-[11px] font-normal">{tag}</Badge>)}
              </div>
            ) : null}
            <div className="flex flex-wrap items-center gap-3">
              <StatChip icon={MessageCircle} value={character.chatCount} label={character.chatCount === 1 ? "chat" : "chats"} />
              <StatChip icon={MessageCircle} value={character.totalMessageCount} label={character.totalMessageCount === 1 ? "message" : "messages"} />
              <StatChip icon={Calendar} value={character.updatedAt.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })} label="updated" />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {!isDemo ? (
            <RowActionsMenu
              ...
              items={[
                { label: "Edit", onSelect: () => void navigate({ to: "/characters/$id/edit", params: { id } }) },
        ...
        <div className="fixed bottom-16 left-0 right-0 z-30 border-t bg-popover/95 px-4 py-3 backdrop-blur-sm md:hidden">
          <Button onClick={handleStartChat} disabled={createChatMutation.isPending} className="w-full">
            <MessageCircle className="size-4" />
            Start Chat
          </Button>
        </div>
      ...
      <div className="lg:grid lg:grid-cols-[200px_1fr] lg:gap-10">
        ...
        <div className="max-w-3xl space-y-12 pt-2">
          <ContentSection id="description" title="Description">
            {data.description ? <MarkdownContent content={data.description} /> : <EmptySection text="No description defined." />}
            {data.creator_notes ? (
              <div className="mt-6">
          ...
          <ContentSection id="personality" title="Personality">
            {data.personality ? <MarkdownContent content={data.personality} /> : <EmptySection text="No personality defined." />}
          </ContentSection>
          <ContentSection id="scenario" title="Scenario">
            {data.scenario ? <MarkdownContent content={data.scenario} /> : <EmptySection text="No scenario defined." />}
          </ContentSection>
          ...
          <ContentSection id="example-messages" title="Example Messages">
            {data.mes_example ? <MarkdownContent content={data.mes_example} /> : <EmptySection text="No example messages defined." />}
          </ContentSection>
          <ContentSection id="prompts" title="Prompts">
            <div className="space-y-6">
              <div>
                <h4 className="text-headline mb-3">System Prompt</h4>
                {data.system_prompt ? <MarkdownContent content={data.system_prompt} /> : <EmptySection text="No system prompt defined." />}
              </div>
              <div>
                <h4 className="text-headline mb-3">Post-History Instructions</h4>
                {data.post_history_instructions ? <MarkdownContent content={data.post_history_instructions} /> : <EmptySection text="No post-history instructions defined." />}
              </div>
            </div>
          </ContentSection>
          <ContentSection id="lorebook" title="Lorebook">
            {data.character_book ? <EmbeddedLorebookPanel book={data.character_book} /> : <EmptySection text="No embedded lorebook." />}
          </ContentSection>
    ...
    <div className="space-y-3">
      ...
      {shown.map((greeting, i) => (
        <div key={i} className="bg-card border rounded-lg p-4 group/copy relative">
          <Badge variant="secondary" className="text-[11px] font-normal mb-2">#{i + 1}</Badge>
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <Button variant="ghost" size="icon" className="size-8" onClick={handleCopy} aria-label={copied ? "Copied" : "Copy to clipboard"}>
      {copied ? <span className="text-success text-[10px] font-medium">✓</span> : <Copy className="size-3.5" />}
    </Button>
  );
}
...
function MetadataGrid({ character }: { character: CharacterDetail }) {
  ...
  const rows: { label: string; value: string }[] = [
    ...
    ...(data.extensions.talkativeness !== undefined ? [{ label: "Talkativeness", value: String(data.extensions.talkativeness) }] : []),
    { label: "Created", value: character.createdAt.toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" }) },
    { label: "Updated", value: character.updatedAt.toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" }) },
    ...(data.tags.length > 0 ? [{ label: "Tags", value: data.tags.map((t) => `#${t}`).join(", ") }] : []),
  ];
  return (
...
function isSectionEmpty(id: string, data: CharacterDetail["data"]): boolean {
  switch (id) {
    case "description": return !data.description && !data.creator_notes && !data.extensions.depth_prompt;
    case "personality": return !data.personality;
    case "scenario": return !data.scenario;
    case "greetings": return !data.first_mes && data.alternate_greetings.length === 0;
    case "example-messages": return !data.mes_example;
    case "prompts": return !data.system_prompt && !data.post_history_instructions;
    case "lorebook": return !data.character_book;
    case "metadata": return false;
    default: return false;
  }
}

function RenameDialog({ character, open, onClose }: { character: CharacterDetail; open: boolean; onClose: () => void }) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (trimmed === character.name) { onClose(); return; }
    setError(null);
    try { await updateMutation.mutateAsync({ id: character.id, name: trimmed }); onClose(); }
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Renames the character record. The card data inside is unchanged.</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="rename">Name</Label>
            <Input id="rename" value={name} onChange={(e) => { setName(e.target.value); setError(null); }} disabled={updateMutation.isPending} autoFocus required />
          </div>
