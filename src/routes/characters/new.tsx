...
function NewCharacterPage() {
  ...
  const handleFileChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setPreviewErr(null);
    if (!f) return;
    if (f.size > 5 * 1024 * 1024) { setPreviewErr("File too large (max 5 MB)."); return; }
    try {
      const b64 = await fileToBase64(f);
      setPreviewB64(b64);
      const res = await previewCharacter({ data: { pngBase64: b64 } });
      if (!res.ok) {
        const msg = res.error.kind === "validation"
    ...
    <main className="mx-auto max-w-[768px] px-4 py-8">
      ...
      {step === "pick" ? (
        <div className="space-y-4">
          <label className="flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed p-12 text-center cursor-pointer hover:bg-surface transition-colors">
            ...
            <input type="file" accept="image/png" onChange={handleFileChange} disabled={importMutation.isPending} className="hidden" />
            <Button type="button" variant="outline" size="sm" asChild><span>Browse files</span></Button>
          </label>
      ...
      {step === "preview" && preview ? (
        <div className="space-y-6">
          <Button variant="ghost" size="sm" onClick={() => { setStep("pick"); setPreview(null); }}><ArrowLeft className="size-4" /> Choose different file</Button>
          {/* Preview card */}
          <div className="rounded-xl border bg-card p-6">
            <div className="flex gap-4">
              {file ? (
                <img src={URL.createObjectURL(file)} alt={preview.preview.name} className="size-32 aspect-[3/4] rounded-lg object-cover border shrink-0" />
              ) : null}
              <div className="min-w-0">
                <h2 className="text-headline">{preview.preview.name}</h2>
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {preview.preview.creator ? <Badge variant="secondary" className="text-xs">{preview.preview.creator}</Badge> : null}
                  <Badge variant="outline" className="text-xs">{preview.preview.spec} v{preview.preview.specVersion}</Badge>
                </div>
                {preview.preview.descriptionExcerpt ? (
                  <p className="text-2 text-sm mt-2 line-clamp-2">{preview.preview.descriptionExcerpt}</p>
                ) : null}
                {preview.preview.tags.length > 0 ? (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {preview.preview.tags.map((t) => <Badge key={t} variant="secondary" className="text-[10px]">{t}</Badge>)}
                  </div>
                ) : null}
                <div className="text-2 text-xs mt-2">
                  {preview.preview.greetingCount} greetings · {preview.preview.lorebookEntryCount} lorebook entries
                </div>
              </div>
            </div>
          </div>
          {preview.preview.warnings.length > 0 ? (
            <div className="rounded-lg border bg-warning/10 p-3 text-xs text-warning space-y-1">
              {preview.preview.warnings.map((w, i) => <p key={i}><AlertTriangle className="size-3 inline" /> {w}</p>)}
            </div>
          ) : null}
          {preview.duplicateOf ? (
            <div className="rounded-lg border bg-warning/10 p-3 text-sm">
              <p><strong className="text-warning">Duplicate detected:</strong> You already have a character named <strong>{preview.duplicateOf.name}</strong>. Importing will create a separate copy.</p>
            </div>
          ) : null}
          {previewErr ? <ErrorBanner message={previewErr} /> : null}
          <div className="flex justify-end gap-2">
            <Button variant="ghost" onClick={() => { setStep("pick"); setPreview(null); }}>Cancel</Button>
